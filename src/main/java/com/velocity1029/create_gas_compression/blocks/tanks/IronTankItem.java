...
public class IronTankItem extends BlockItem {

    public IronTankItem(Block p_i48527_1_, Properties p_i48527_2_) {
    ...
    protected boolean updateCustomBlockEntityTag(BlockPos p_195943_1_, Level p_195943_2_, Player p_195943_3_,
        ...
        if (nbt != null) {
            ...
            if (nbt.contains("TankContent")) {
                ...
                if (!fluid.isEmpty()) {
                    fluid.setAmount(Math.min(IronTankBlockEntity.getCapacityMultiplier(), fluid.getAmount()));
    ...
    private void tryMultiPlace(BlockPlaceContext ctx) {
        ...
        if (!IronTankBlock.isTank(placedOnState))
            return;
...
//                creative ? AllBlockEntityTypes.CREATIVE_FLUID_TANK.get() : AllBlockEntityTypes.FLUID_TANK.get(), world, placedOnPos
        IronTankBlockEntity tankAt = ConnectivityHandler.partAt(CGCBlockEntities.IRON_TANK.get(), world, placedOnPos
        );
        if (tankAt == null)
            return;
